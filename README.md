...
Плюс для локальной проверки: **тестовый вход по любой почте без пароля** (`AUTH_DEV_LOGIN_ENABLED`), пользователь создаётся автоматически, по умолчанию с ролью editor.

Все три вещи выключены по умолчанию и включаются переменными окружения.
...
- [datalens/docs/dev-login.md](datalens/docs/dev-login.md): тестовый вход по почте, запуск всего стека из исходников форков одной командой, пошаговый сценарий проверки прав в двух окнах браузера.
- [datalens/README.md](datalens/README.md): штатная инструкция по запуску DataLens, в конце разделы про SSO, права и тестовый вход.
...
## Локальный стенд для проверки прав

Пошаговая инструкция с нуля, для коллег: [QUICKSTART.md](QUICKSTART.md). Коротко:

Стек поднимается прямо из исходников форков, образы собирать не нужно. В `datalens/.env`:
...
`--dev-root` на Linux и WSL обязателен, иначе контейнеры не могут писать в каталоги форков. Интерфейс на http://localhost:8080. Админ входит по паролю из `.env`, остальные через блок «Тестовый вход» на странице входа: любая почта, без пароля, учётка создаётся сама. Сценарий проверки по шагам в [datalens/docs/dev-login.md](datalens/docs/dev-login.md). Тестовый вход нельзя включать на установке с реальными данными: войти можно кем угодно.
